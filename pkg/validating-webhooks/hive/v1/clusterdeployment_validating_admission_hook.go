...
func validateClusterPlatform(path *field.Path, platform hivev1.Platform) field.ErrorList {
	...
	if powervs := platform.PowerVS; powervs != nil {
		...
		if powervs.CredentialsSecretRef.Name == "" {
			allErrs = append(allErrs, field.Required(powerVSCloudPath.Child("credentialsSecretRef", "name"), "must specify secrets for IBM PowerVS access"))
		}
		if powervs.Region == "" {
			allErrs = append(allErrs, field.Required(powerVSCloudPath.Child("region"), "must specify IBM PowerVS region"))
		}
	}
