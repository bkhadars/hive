...
type API interface {
	...
	// TG
	GetTGConnectionVPC(ctx context.Context, vpcID string) (string, error)
	...
	// Service Instance
	ListServiceInstances(ctx context.Context, resourceGroup string) ([]string, error)
	ServiceInstanceGUIDToName(ctx context.Context, id, resourceGroup string) (string, error)
	ServiceInstanceNameToGUID(ctx context.Context, name, resourceGroup string) (string, error)
...
func (c *Client) ListResourceGroups(ctx context.Context) (*resourcemanagerv2.ResourceGroupList, error) {
	...
	listResourceGroupsOptions.SetAccountID(*apikey.AccountID)
...
func (c *Client) ListServiceInstances(ctx context.Context, resourceGroup string) ([]string, error) {
	var (
	...
	listGroupOptions.SetAccountID(*apikey.AccountID)
...
func (c *Client) ServiceInstanceGUIDToName(ctx context.Context, id, resourceGroup string) (string, error) {
	var (
		options   *resourcecontrollerv2.ListResourceInstancesOptions
		resources *resourcecontrollerv2.ResourceInstancesList
		err       error
	...
	for _, group := range groups.Resources {
		if *group.Name == rgroupID {
			rgroupID = *group.ID
		}
	}

	options = c.controllerAPI.NewListResourceInstancesOptions()
	options.SetResourceGroupID(rgroupID)
	// resource ID for Power Systems Virtual Server in the Global catalog
	options.SetResourceID(powerIAASResourceID)
	options.SetLimit(perPage)
...
func (c *Client) ServiceInstanceNameToGUID(ctx context.Context, name, resourceGroup string) (string, error) {
	var (
		options   *resourcecontrollerv2.ListResourceInstancesOptions
		resources *resourcecontrollerv2.ResourceInstancesList
		err       error
...
func (c *Client) GetAttachedTransitGateway(ctx context.Context, svcInsID string) (string, error) {
	...
	for range gateways {
		conns, err = c.getTransitConnections(ctx)
		if err != nil {
			return "", err
		}
...
func (c *Client) GetTGConnectionVPC(ctx context.Context, vpcID string) (string, error) {
	conns, err := c.getTransitConnections(ctx)
	if err != nil {
		return "", err
	}
	for _, conn := range conns {
		if *conn.NetworkType == "vpc" && strings.Contains(*conn.NetworkID, vpcID) {
...
func (c *Client) getTransitConnections(ctx context.Context) ([]transitgatewayapisv1.TransitConnection, error) {
	var (
...
func GetCISInstanceCRN(client API, ctx context.Context, baseDomain string) (string, error) {
	/*var publish types.PublishingStrategy
	if !internal {
